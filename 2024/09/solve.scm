  ...
  (define (iter block-map file-blocks free-blocks file-idx)
    (if (and (null? file-blocks) (null? free-blocks))
        ...
        (iter (append block-map
              ...
              (1+ file-idx))))
  (iter (list) file-blocks free-blocks 0))
...
(define (defragmented? block-map)
  (let ((idx-start (list-index empty-block? block-map)))
    (every empty-block? (drop block-map idx-start))))
...
(define (defragment block-map)
  (if (defragmented? block-map)
      ...
      (let* ((i1 (get-first-empty-block-idx block-map))
             ...
             (v (list-ref block-map i1)))
        (list-set! block-map i1 (list-ref block-map i2))
        (list-set! block-map i2 v)
        (defragment block-map))))

(define (get-first-empty-block-idx block-map)
  (list-index empty-block? block-map))

;; TODO: Optimise (length, reverse)!
(define (get-last-non-empty-block-idx block-map)
  (- (length block-map)
     (list-index (compose not empty-block?) (reverse block-map))
     1))
